    ...
    password = fields.CharField(
        ...
        widget=widgets.PasswordInput(
            attrs={'class': 'form-control',
                   'placeholder': _('Mot de passe'), 'autocomplete': 'current-password'}
        )
    )
...
class UserSignupForm(auth_forms.UserCreationForm):
    password1 = forms.CharField(
        widget=forms.PasswordInput(
            attrs={'class': 'form-control', 'autocomplete': 'new-password', 'placeholder': 'Mot de passe'}),
        strip=False,
    )
    password2 = forms.CharField(
        widget=forms.PasswordInput(
            attrs={'class': 'form-control', 'autocomplete': 'new-password', 'placeholder': 'Confirmation du mot de passe'}),
        strip=False
    )
    ...
    def clean(self):
        ...
        if len(password1) < 10:
            raise forms.ValidationError(
                'Votre mot de passe doit comporter au moins 10 charactères')
...
class CustomSetPasswordForm(auth_forms.SetPasswordForm):
    ...
    new_password2 = forms.CharField(
        ...
        widget=forms.PasswordInput(
            attrs={'class': 'form-control',
                   'placeholder': 'Nouveau mot de passe confirmation', 'autocomplete': 'off'}
        ),
        strip=False,
    )
